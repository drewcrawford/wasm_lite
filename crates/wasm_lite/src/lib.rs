...
//!   cannot block. Its opt-in `fs` feature adds asynchronous read-only files:
//!   pooled `fs` operations natively and HTTP range reads in the browser.
...
//!   feature when `wasm_lite` owns final codegen; bounded `[patch]` shims exist
