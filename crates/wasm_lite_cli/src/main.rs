    ...
    match args.output {
        Some(path) => {
            ...
            if memory.is_some() {
                ...
                // Overwrite a previously generated bootstrap freely, but never
                // clobber an unrelated file that happens to share the name.
                if let Ok(existing) = std::fs::read_to_string(&worker_path)
                    && !existing.starts_with("// Generated by wasm_lite_codegen")
                {
                    return Err(format!(
                        "{} already exists and was not generated by wasm-lite; \
                         move it or choose a different output directory",
                        worker_path.display()
                    ));
                }
